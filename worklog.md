...
## Sprint: Página de Setup Automático (2026-04-24)

### Objetivo
Implementar página de setup automático que detecta se o banco de dados está configurado e guia o usuário pela configuração inicial do sistema.
...
- Retorna status completo com detalhes do banco

#### 2. API de Setup Inicial
**File:** `/src/app/api/setup/route.ts`

**Features:**
- Recebe configurações do banco (DATABASE_URL, NEXTAUTH_SECRET, etc.)
- Testa conexão com o banco
- Executa `prisma db push` via programação
- Cria empresa inicial
- Cria usuário admin com senha criptografada
- Salva arquivo de configuração local
- Retorna instruções claras para configurar variáveis no Vercel

**Validações:**
- URL do banco (postgresql://, mysql://, sqlite:)
- Email válido
...
**Features:**
- Design moderno e profissional (igual ao login)
- Verificação automática de status ao carregar
- Formulário completo com:
  - URL do banco de dados (com botão mostrar/ocultar)
  - Chave secreta NextAuth (com geração automática)
  - URL do sistema (pré-preenchida)
  - Dados da empresa (nome, CNPJ, email)
  - Dados do administrador (nome, email, senha)
  - Integração MercadoPago (opcional)
- Progresso visual durante o processo
- Seções expandíveis para ajuda
- Instruções para Vercel após setup completo

#### 4. Middleware Atualizado
**File:** `/src/middleware.ts`

**Features:**
- Detecta se o sistema está configurado (verifica DATABASE_URL)
- Redireciona para /setup se não configurado
- Permite rotas públicas e de API de setup
- Permite arquivos estáticos

### Fluxo do Setup
1. Usuário acessa o site
2. Sistema detecta que não está configurado
3. Redireciona para /setup
4. Usuário preenche os dados
5. Sistema testa conexão
6. Sistema cria as tabelas
7. Sistema cria a empresa e usuário admin
8. Sistema mostra instruções para Vercel
9. Usuário configura variáveis no Vercel
10. Sistema redireciona para login

### Arquivos Criados/Modificados
- `/src/app/api/setup/status/route.ts` - API de verificação de status
- `/src/app/api/setup/route.ts` - API de setup inicial
- `/src/app/setup/page.tsx` - Página de setup
- `/src/middleware.ts` - Middleware com detecção de setup

### Status
- ✅ Build passando (Next.js 16)
- ✅ TypeScript sem erros críticos
- ✅ Sistema de setup completo e funcional
