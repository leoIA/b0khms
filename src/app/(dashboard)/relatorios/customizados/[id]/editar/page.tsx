...
// Form component - receives initial data as props
function ReportEditForm({ 
  ...
  const [category, setCategory] = useState<ReportCategory>(initialData.category)
  const [type, setType] = useState<ReportType>(initialData.type)
  const [dataSource] = useState<DataSource>(initialData.dataSource)
  const [isPublic, setIsPublic] = useState(initialData.isPublic)
  const [isDefault, setIsDefault] = useState(initialData.isDefault)
  const [cacheEnabled, setCacheEnabled] = useState(initialData.cacheEnabled)
  const [cacheDuration, setCacheDuration] = useState(initialData.cacheDuration)
  const [filters, setFilters] = useState<FilterConfig[]>(initialData.filters)
  const [sorts, setSorts] = useState<SortConfig[]>(initialData.sorts)
  const [limit, setLimit] = useState(initialData.limit)
  const [columns, setColumns] = useState<ColumnConfig[]>(initialData.columns)
  const [chartType, setChartType] = useState<ChartType>(initialData.chartType)
  const [chartXAxis, setChartXAxis] = useState(initialData.chartXAxis)
  const [chartYAxis, setChartYAxis] = useState<string[]>(initialData.chartYAxis)
  const [showLegend, setShowLegend] = useState(initialData.showLegend)
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      router.push(`/relatorios/customizados/${reportId}`)
    },
    onError: (error: Error) => {
      toast({
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Editar Relatório</h1>
            <p className="text-muted-foreground">{name}</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => router.push(`/relatorios/customizados/${reportId}`)}>
            Cancelar
          </Button>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="p-3 bg-muted/50 rounded-lg">
                <p className="text-sm text-muted-foreground">
                  <strong>Fonte de Dados:</strong> {DATA_SOURCES.find(ds => ds.value === dataSource)?.label || dataSource}
                </p>
                <p className="text-xs text-muted-foreground mt-1">
                  A fonte de dados não pode ser alterada após a criação.
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent>
              <Tabs defaultValue="filters">
                <TabsList>
                  ...
                  {type === 'chart' && <TabsTrigger value="chart">Gráfico</TabsTrigger>}
                </TabsList>

                {/* Filters Tab */}
                <TabsContent value="filters" className="space-y-4 mt-4">
                  <div className="flex items-center justify-between">
                    <p className="text-sm text-muted-foreground">Adicione filtros para refinar os dados</p>
                    ...
                    <div className="space-y-3">
                      {filters.map((filter, index) => (
                        <div key={index} className="flex items-end gap-2 p-3 border rounded-lg">
                          ...
                          <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" onClick={() => removeFilter(index)}>
                ...
                <TabsContent value="sort" className="space-y-4 mt-4">
                  <div className="flex items-center justify-between">
                    <p className="text-sm text-muted-foreground">Defina a ordem dos resultados</p>
                      ...
                      {sorts.map((sort, index) => (
                        <div key={index} className="flex items-end gap-2 p-3 border rounded-lg">
                          <div className="flex items-center gap-1">
                            <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => moveSort(index, 'up')} disabled={index === 0}>
                              <ChevronUp className="h-4 w-4" />
                            </Button>
                            <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => moveSort(index, 'down')} disabled={index === sorts.length - 1}>
                          ...
                          <div className="flex-1 grid grid-cols-2 gap-2">
                            ...
                            <div className="space-y-1">
                              ...
                              <Select value={sort.direction} onValueChange={(v) => updateSort(index, { direction: v as 'asc' | 'desc' })}>
                                <SelectTrigger className="h-8">
                                  <SelectValue />
                                </SelectTrigger>
                                <SelectContent>
                          ...
                          <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" onClick={() => removeSort(index)}>
                            <Trash2 className="h-4 w-4" />
                          </Button>
                        </div>
                      ))}
                  ...
                  <div className="space-y-2 pt-4">
                    ...
                    <Input type="number" value={limit} onChange={(e) => setLimit(Number(e.target.value))} min={1} max={10000} className="w-32" />
                  </div>
                </TabsContent>

                {/* Columns Tab */}
                <TabsContent value="columns" className="space-y-4 mt-4">
                  <p className="text-sm text-muted-foreground">Configure as colunas que serão exibidas</p>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Visível para todos da empresa</p>
                </div>
                <Switch checked={isPublic} onCheckedChange={setIsPublic} />
              </div>
              <div className="flex items-center justify-between">
                <div>
                  <Label>Relatório Padrão</Label>
                  <p className="text-sm text-muted-foreground">Exibido como padrão na categoria</p>
                </div>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">Armazena resultados em cache</p>
                </div>
              ...
              {cacheEnabled && (
                <div className="space-y-2">
                  ...
                  <Input type="number" value={cacheDuration} onChange={(e) => setCacheDuration(Number(e.target.value))} min={60} max={86400} />
                </div>
              )}
            </CardContent>
          </Card>
...
// Main page component - handles loading and error
export default function EditarRelatorioPage() {
  ...
  const initialData = {
    ...
    cacheEnabled: report.cacheEnabled ?? true,
    cacheDuration: report.cacheDuration ?? 300,
    filters: report.queryConfig?.filters || [],
    sorts: report.queryConfig?.sort || [],
    limit: report.queryConfig?.limit || 1000,
    columns: report.columnConfig || [],
    chartType: report.chartConfig?.type || 'bar',
    chartXAxis: report.chartConfig?.xAxis || '',
    chartYAxis: report.chartConfig?.yAxis || [],
    showLegend: report.chartConfig?.showLegend ?? true,
  }

  return <ReportEditForm initialData={initialData} reportId={reportId} />
}
